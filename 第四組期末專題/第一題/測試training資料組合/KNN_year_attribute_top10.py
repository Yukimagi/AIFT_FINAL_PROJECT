...
import json
columns_comb1=['股價淨值比', '股價營收比', 'M營業利益成長率', 'M稅後淨利成長率']
columns_comb2=['股價淨值比', 'M淨值報酬率─稅後', '資產報酬率ROA', 'M存貨週轉率 (次)', 'M應收帳款週轉次', 'M稅後淨利成長率']
columns_comb3=['股價淨值比', '利潤邊際NPM', 'M存貨週轉率 (次)', 'M應收帳款週轉次', 'M營業利益成長率', 'M稅後淨利成長率']
columns_comb4=['股價營收比', '營業利益率OPM', 'M存貨週轉率 (次)', 'M應收帳款週轉次', 'M營業利益成長率', 'M稅後淨利成長率']
columns_comb5=['本益比', '股價淨值比', '股價營收比', 'M淨值報酬率─稅後', '資產報酬率ROA', 'M存貨週轉率 (次)', 'M營業利益成長率']
columns_comb6=['股價淨值比', '股價營收比', 'M淨值報酬率─稅後', '資產報酬率ROA', 'M流動比率', 'M速動比率', 'M應收帳款週轉次']
columns_comb7=['本益比', '股價淨值比', '股價營收比', '資產報酬率ROA', '營業利益率OPM', '利潤邊際NPM', 'M流動比率', 'M應收帳款週轉次', 'M營業利益成長率']
columns_comb8=['本益比', '股價淨值比', '股價營收比', 'M淨值報酬率─稅後', '資產報酬率ROA', '利潤邊際NPM', '負債/淨值比', 'M流動比率', 'M速動比率', 'M營業利益成長率', 'M稅後淨利成長率']
columns_comb9=['股價營收比', '營業利益率OPM', '負債/淨值比', 'M速動比率', 'M存貨週轉率 (次)', 'M營業利益成長率', 'M稅後淨利成長率']
columns_comb10=['本益比', '股價淨值比', '股價營收比', '營業利益率OPM', '利潤邊際NPM', 'M流動比率', 'M速動比率', 'M存貨週轉率 (次)', 'M應收帳款週轉次', 'M營業利益成長率', 'M稅後淨利成長率']
...
with open('KNN_train_test_result_30000.txt', 'a') as file:
    for key, selected_years in selected_range:
        ...
        for col in range (1,11):#取top10組合
            cols=cols+1
            list_ans=[]
            # 指定要選擇的欄位
        ...
        #--------------------------------------------------------------------------------------------------
            ...
            for year in test_years:
                ...
                print(selects_ret)
                with open(f'selected_stock_KNN_train_30000/{cols}.txt', 'a') as stockfile:
            ...
            if(sum2>max_ret):
                max_ret=sum2
                with open('max_KNN_train_30000.txt', 'w') as maxfile:
